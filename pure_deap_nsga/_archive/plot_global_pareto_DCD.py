"""
DC-D | 三目标两两全局帕累托前沿可视化（6 张图）
工况：Cold-plate liquid (Summer), T_hs=55°C, T_cs=25°C, ΔT=30 K
"""
...
WP_TAG   = 'DCD'
WP_SUB   = r'Cold-plate Summer, $T_{hs}$=55°C, $T_{cs}$=25°C, $\Delta T$=30 K'
...
# ── 数据加载 ──────────────────────────────────────────────────────────────────
files = sorted(glob.glob(f'{RESULTS_DIR}/pareto_DC-D_*.csv'))
all_items = []
for f in files:
    stem  = os.path.basename(f).replace('pareto_DC-D_','').replace('.csv','')
    parts = stem.split('_')
...
print(f'DC-D pool: {len(pool)} solutions, {len(files)} files')
...
for P in PAIRS:
    ...
    fig.suptitle(f'DC-D  |  Individual and global Pareto fronts\n'
    ...
    for item in all_items:
        ...
        if n_global<30:
            ax.step(qx,qy,where='post',color=CFG_STYLE[item['cb']]['color'],
    ...
    if n_global<30:
        ...
        sub=pool[pool['dT_st_sp']<=30]
        ax.scatter(sub[xc]*xs,sub[yc]*ys,s=8,color='#444',alpha=0.12,
                   zorder=1,edgecolors='none')
    else:
        ax.plot(gx,gy,**GLOBAL_STYLE,solid_capstyle='round')

    ax.text(gx.max()-0.3,gy[np.argmax(gx)]-1.5,'Global Pareto front',
    ...
    ax.text(gx.max()+0.2,y_lo+0.3,f'max={gx.max():.1f} {P["xunit"]}',
    ...
    fig2.suptitle(f'DC-D  |  Fluid pair contributions within each CB configuration\n'
    ...
    for ax2,cb in zip(axes.flat,CFG_STYLE):
        ...
        if len(nd_pool)<2:
            ax2.set_title(f"{cb_st['label']}\n(no solutions)",fontsize=9.5,color='#aaa')
        ...
        ax2.scatter(px[-1],py[-1],color=cb_st['color'],s=70,zorder=10,edgecolors='white',linewidths=1.0)
        idx_D=np.argmax(py)
        ...
        hp_h=[mlines.Line2D([],[],color=HP_COLOR[f],lw=1.8,label=f'HP: {f}')
              for f in hp_f if f in HP_COLOR]
