"""
DC-C | 三目标两两全局帕累托前沿可视化（6 张图）
工况：Cold-plate liquid (Winter), T_hs=50°C, T_cs=5°C, ΔT=45 K
对应三组目标对：
...
WP_TAG   = 'DCC'
WP_TITLE = r'DC-C  |  Cold-plate liquid (Winter)  $T_{hs}$=50°C, $T_{cs}$=5°C, $\Delta T$=45 K'
...
PAIRS = [
    dict(
        ...
        label_pos={
            ...
            'SRVCHP_SBORC': (0.30,  4.0,  3.0),
            'SBVCHP_SRORC': (0.55,  4.0, -3.0),
            'SRVCHP_SRORC': (0.72,  4.0,  3.0),
        },
    ),
    dict(
        ...
        label_pos={
            'SBVCHP_SBORC': (0.20, -4.0, -2.5),
            'SRVCHP_SBORC': (0.35,  4.0,  2.5),
            'SBVCHP_SRORC': (0.55, -4.0,  2.5),
            'SRVCHP_SRORC': (0.75,  4.0, -2.5),
        },
    ),
    dict(
        ...
        label_pos={
            'SBVCHP_SBORC': (0.15, -4.0, -3.0),
            'SRVCHP_SBORC': (0.35,  4.0,  3.0),
            'SBVCHP_SRORC': (0.55,  4.0, -3.0),
            'SRVCHP_SRORC': (0.75,  4.0,  3.0),
        },
    ),
]
...
# ── 数据加载 ──────────────────────────────────────────────────────────────────
files = sorted(glob.glob(f'{RESULTS_DIR}/pareto_DC-C_*.csv'))
all_items = []
for f in files:
    stem  = os.path.basename(f).replace('pareto_DC-C_', '').replace('.csv', '')
    parts = stem.split('_')
...
print(f'DC-C pool: {len(pool)} solutions, {len(files)} files')
...
for P in PAIRS:
    ...
    fig.suptitle(
        f'DC-C  |  Individual and global Pareto fronts\n'
        f'Objectives: {P["title_pair"]}  '
        r'(Cold-plate Winter, $T_{hs}$=50°C, $T_{cs}$=5°C, $\Delta T$=45 K)',
    ...
    for cb, nd_cb in cfg_pool_nd.items():
        ...
        ax.plot(px, py, color=st['color'], lw=lw_eff, ls=ls_eff,
    ...
    if n_global < 30:
        ...
        eff_mask = pool['dT_st_sp'] <= 30
        sub = pool[eff_mask]
        ax.scatter(sub[xc] * xs, sub[yc] * ys,
                   s=8, color='#444', alpha=0.12, zorder=1, edgecolors='none')
    else:
    ...
    ax.text(gx.max() + 0.2, y_lo + 0.3,
            f'max={gx.max():.1f} {P["xunit"]}', ha='left', va='bottom',
            fontsize=7.5, color='#666')
    ax.text(x_lo + 0.3, gy.max() + 0.3,
            f'max={gy.max():.2f} {P["yunit"]}', ha='left', va='bottom',
            fontsize=7.5, color='#666')
    ax.text(0.98, 0.02,
            f'Global front: n={n_global}\nSpearman r = {r_g:.3f}',
    ...
    handles = [mlines.Line2D([], [], color=st['color'], lw=st['lw'],
                             label=st['label']) for cb, st in CFG_STYLE.items()]
    handles += [mlines.Line2D([], [], color='#111', lw=3.0,
                              label='Global Pareto front'),
                mlines.Line2D([], [], color='#aaa', lw=0.6,
    ...
    fig2.suptitle(
        f'DC-C  |  Fluid pair contributions within each CB configuration\n'
    ...
    for ax2, cb in zip(axes.flat, CFG_STYLE):
        ...
        x_lo2 = max(0, px.min() - 1); x_hi2 = px.max() + 5
        ...
        best_x_row = nd_pool.loc[nd_pool[xc].idxmax()]
        best_y_row = nd_pool.loc[nd_pool[yc].idxmax()]
        ax2.annotate(
            f'max {P["xn"]}:\n{best_x_row["fluid_hp"]}/{best_x_row["fluid_he"]}',
            xy=(px[-1], py[-1]), xytext=(px[-1] - 2.5, py[-1] + 1.5),
        ...
        ax2.annotate(
            f'max {P["yn"]}:\n{best_y_row["fluid_hp"]}/{best_y_row["fluid_he"]}',
