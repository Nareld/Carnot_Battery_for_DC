#!/usr/bin/env python3
"""
CBSim Carnot Battery Multi-Objective Optimization - Position position1
...
import pathlib
_CBSIM_ROOT = pathlib.Path(__file__).resolve().parents[6]  # CBSim_clean root
sys.path.insert(0, str(_CBSIM_ROOT / "src"))
...
def set_params():
    ...
    """
    # Fixed parameters for CBSim model - Position position1
...
def evaluate(x_in, params=[]):
    ...
    try:
        ...
        # Fixed parameters for position1 (from design_space.csv 'par' entries)
        T_hp_cs = design_vars.get('T_hp_cs', 15.0)      # Heat source temperature (°C)
...
if __name__ == "__main__":
    ...
    print("Testing CBSim multi-objective case description for position1...")
