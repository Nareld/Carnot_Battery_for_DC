#!/usr/bin/env python3
"""
CBSim Carnot Battery Multi-Objective Optimization Case Description

This module defines the case description for CBSim multi-objective optimization in RHEIA framework.
Three-objective optimization targeting:
1. cb_efficiency (maximize)
2. E_dens_th (maximize)
3. eta_cb_exer (maximize)
"""
...
def evaluate(x_in, params=[]):
    ...
    try:
        ...
        if success:
            ...
            cb_efficiency, hp_cop, he_efficiency = model.get_multi_objective_values()

            # RHEIA minimizes objectives, so we return negative values to maximize them
            return (-cb_efficiency, -hp_cop, -he_efficiency)
        else:
...
if __name__ == "__main__":
    ...
    print("Testing CBSim multi-objective case description...")
