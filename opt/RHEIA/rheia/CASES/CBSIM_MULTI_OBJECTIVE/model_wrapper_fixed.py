#!/usr/bin/env python3
"""
Fixed CBSim Model Wrapper for RHEIA Integration
Based on working implementation from feasibility_checker_complete.py
"""
...
class CBSimModelFixed:
    """Fixed wrapper class for CBSim thermal model"""
    ...
    def _extract_results_with_constraints(self, start_time):
        ...
        try:
            ...
            if self.model is not None:
                ...
                results['cb_efficiency'] = getattr(self.model, 'eta_cb_elec', 0.0)

                # Heat pump performance
                results['hp_cop'] = getattr(self.model.my_HP, 'eta_hp_cyclen', 0.0)
                results['hp_power'] = getattr(self.model, 'P_hp', 0.0)

                # Heat engine performance
                results['he_efficiency'] = getattr(self.model.my_HE, 'eta_he_cyclen', 0.0)
                results['he_power'] = getattr(self.model, 'P_he', 0.0)

                # Temperature constraints
                results['T_hp_ev'] = getattr(self.model.my_HP, 'T_hp_5', 0.0)
                results['T_hp_cd'] = getattr(self.model.my_HP, 'T_hp_2', 0.0)
                results['T_he_ev'] = getattr(self.model.my_HE, 'T_he_5', 0.0)
                results['T_he_cd'] = getattr(self.model.my_HE, 'T_he_2', 0.0)
            else:
...
if __name__ == "__main__":
    """Test the fixed model wrapper"""
    print("Testing Fixed CBSim model wrapper...")

    # Create model instance
    model = CBSimModelFixed()
