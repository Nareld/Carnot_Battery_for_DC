...
class CBSimModel:
    ...
    def get_multi_objective_values(self):
        ...
        return (
            self.results.get('cb_efficiency', 0.0),
            self.results.get('E_dens_th', 0.0),
            self.results.get('eta_cb_exer', 0.0)
        )
    ...
    def _extract_results(self, start_time):
        ...
        try:
            ...
            if self.model is not None:
                ...
                results['E_dens_th'] = getattr(self.model, 'E_dens_th', 0.0)
                results['eta_cb_exer'] = getattr(self.model, 'eta_cb_exer', 0.0)
            else:
