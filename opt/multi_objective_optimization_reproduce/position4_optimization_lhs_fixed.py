#!/usr/bin/env python3
"""
卡诺电池多目标优化主脚本 - LHS采样版本（修正版）针对 position4
...
针对 position4 (t_hs=100°C, t_cs=15°C) 和新的设计空间（11个变量）
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('position4_optimization_lhs_fixed.log'),
...
class Position4MultiObjectiveOptimizerLHSFixed:
    """多目标优化器类 - LHS采样版本（修正版）针对 position4"""
    ...
    def setup_optimization(self):
        ...
        """
        opt_params = {
            'case': 'CBSIM_POSITION_POSITION4',  # 使用 position4 case
            'objectives': {
                'DET': (-1, -1, -1)  # 三目标优化，全部最大化
            },
            'stop': ('BUDGET', 6000),      # 50次评估（测试用）
            'population size': 100,       # 种群大小20（测试用）
            'n jobs': 1,                  # 单进程
            'results dir': 'position4_multi_objective_results_lhs_fixed_test',
    ...
    def run_optimization(self):
        """运行多目标优化"""
        try:
            logger.info("开始 position4 多目标优化 - LHS采样版本（修正版）")
            ...
            # 检查case目录是否存在
            case_dir = os.path.join(rheia_dir, 'rheia', 'CASES', 'CBSIM_POSITION_POSITION4')
            ...
            logger.info(f"position4 多目标优化完成，耗时: {optimization_time:.2f}s")
            ...
            logger.error(f"position4 多目标优化失败: {e}")
    ...
    def _extract_pareto_front(self, opt_params, optimization_time):
        ...
        try:
            # 从RHEIA结果目录提取帕累托前沿
            rheia_results_dir = _pathlib.Path(__file__).resolve().parent.parent / 'RHEIA' / 'rheia' / 'RESULTS' / 'CBSIM_POSITION_POSITION4' / 'DET' / opt_params['results dir']
    ...
    def save_results(self):
        ...
        try:
            # 创建结果目录
            results_dir = Path('position4_multi_objective_results_lhs_fixed')
            ...
            logger.info(f"position4 多目标优化结果已保存到: {results_dir}")
...
def create_default_config(config_file):
    ...
    config = {
        "multi_objective_optimization": {
            "description": "卡诺电池多目标优化配置 - position4 (t_hs=100°C, t_cs=15°C)",
            "author": "CBSim Optimization Team",
            "version": "1.0",
            "date": "2025-12-04",
            "position_id": "position4",
            "position_name": "位置4 (t_hs=100°C, t_cs=15°C, ΔT_hs-cs=85K)"
        },
        ...
        "output_settings": {
            "results_directory": "position4_multi_objective_results",
...
def main():
    """主函数"""
    try:
        # 配置文件路径 - 使用position4专用配置文件
        script_dir = Path(__file__).parent
        config_file = script_dir / 'position4_multi_objective_config.json'
        ...
        # 创建优化器实例
        optimizer = Position4MultiObjectiveOptimizerLHSFixed(str(config_file))

        # 运行多目标优化
        success = optimizer.run_optimization()

        if success:
            logger.info("position4 多目标优化分析全部完成 - LHS修正版本")
        else:
            logger.error("position4 多目标优化失败")

    except Exception as e:
        logger.error(f"position4 多目标优化分析失败: {e}")
        sys.exit(1)
